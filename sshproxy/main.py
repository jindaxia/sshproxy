...
def set_connection():
    ...
    try:
        ...
        print(
            "Now forwarding remote port %d to %s:%d ..." % (port, remote[0], remote[1])
        )
...
def build_jump(bastion, des_server, des_port):
    ...
    try:
        print("*** connecting to bastion %s:%d: %r" % (bastion[0], bastion[1], e))
        ...
        jumpbox_channel = bastion_transport.open_channel("direct-tcpip", dest_addr, src_addr)
    except Exception as e:
        print("*** Failed to connect to bastion %s:%d: %r" % (bastion[0], bastion[1], e))
        return None
...
def setup_wget_proxy(client, port):
    stdin, stdout, stderr = client.exec_command("sed -i 's/^#\?http_proxy = .*$/http_proxy = http:\/\/localhost:10888\//g' /etc/wgetrc")
    if len(stderr.readlines())> 0:
        print("ERROR set wget proxy ")
    stdin, stdout, stderr = client.exec_command("sed -i 's/^#\?https_proxy = .*$/https_proxy = http:\/\/localhost:10888\//g' /etc/wgetrc")
